...
class TransPoseR(nn.Module):
    ...
    def __init__(self, block, layers):
        ...
        n_head = 8
        pos_embedding_type_ours = 'sine'
        w, h = [1024, 1024]

        self.reduce = nn.Conv2d(self.inplanes, d_model, 1, bias=False)
        self._make_position_embedding(w, h, d_model, pos_embedding_type_ours)
    ...
    def _make_position_embedding(self, w, h, d_model, pe_type='sine'):
        ...
        if pe_type == 'none':
            ...
            if pe_type == 'learnable':
                self.pos_embedding_train = nn.Parameter(
    ...
    def forward(self, x):
        ...
        x = self.global_encoder(x, pos=self.pos_embedding_train)
        x = x.permute(1, 2, 0).contiguous().view(bs, c, h, w)
        x = self.deconv_layers(x)
        x1 = self.adj_conv1(x)
        x2 = self.adj_conv2(self.avgpool2(x))
        x3 = self.adj_conv3(self.avgpool3(x))
        x = [x1, x2, x3]
        return x
