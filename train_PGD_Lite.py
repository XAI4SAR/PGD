...
from models.PGD_Lite_yolo import Model
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    kpweights_Transpose = opt.kpweights_Transpose
    ...
    Transpose_pretrain=kpweights_Transpose
    if pretrained:
        with torch_distributed_zero_first(LOCAL_RANK):
            weights = attempt_download(weights)  # download if not found locally
        ckpt = torch.load(weights, map_location='cpu')  # load checkpoint to CPU to avoid CUDA memory leak
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors'), device=device).to(device)  # create 修改多传入device参数
    ...
    if Transpose_pretrain:
        weights_dict = torch.load(Transpose_pretrain, map_location='cpu')
        weights_dict = weights_dict if 'model' not in weights_dict else weights_dict['model']
        missing_keys, unexpected_keys=model.Transposemodel.load_state_dict(weights_dict, strict=False)

    freeze = [f'model.{x}.' for x in (freeze if len(freeze) > 1 else range(freeze[0]))]  # layers to freeze
    #freeze = ['Transposemodel.']
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%10s' * 8) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'loc','labels', 'img_size'))
        if RANK in {-1, 0}:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--kpweights_Transpose', type=str, default='', help='initial keypoints weights path')
