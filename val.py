...
def run(
    ...
    if not training:
        ...
        pad = 0.0 if task in ('speed', 'benchmark') else 0.0  ##  修改：0.5修改成0.0了
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Inference
        out, train_out ,loc_out= model(im) if training else model(im, augment=augment, val=True)  # inference, loss outputs
        dt[1] += time_sync() - t2
        ...
        init()
        set_value('input_img',im)
        if compute_loss:
                train_out=(train_out,loc_out)
